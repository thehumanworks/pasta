...
- [ ] **DoD-6** — TypeScript compiles; Worker changes are deployed with a non-leaking remote smoke when remote deploy is available. — *verify by:* `mise exec -- bunx tsc --noEmit` and deploy/smoke evidence or recorded blocker
...
### T5 · Verify, Deploy, Checkpoint · [ ]

**Steps**
- [x] Run TypeScript compile and focused test suites.
- [ ] Deploy Worker and run non-leaking remote smoke when credentials allow.
- [ ] Record evidence and mark DoD/tasks complete.
