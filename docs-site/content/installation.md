...
bunx --bun github:thehumanworks/pasta#v0.1.14 --version
```
...
| `PASTA_COPY_KEY` | Default `install-hotkeys` / `install-shell` copy key when `--copy-key` is absent |
| `PASTA_PASTE_KEY` | Default `install-hotkeys` / `install-shell` paste key when `--paste-key` is absent |
