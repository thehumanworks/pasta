...
- Put the Pasta row in KeyboardKit's **native `toolbar:` slot** and keep the
  stock autocomplete view:
  `toolbar: { params in PastaKeyboardToolbar(autocompleteToolbar: params.view, ...) }`.
  KeyboardKit already composes the keyboard as `VStack { toolbar; keys }`; replacing
  `params.view` with a custom `Publish`/`Paste` shelf removes autocorrect.
- Do **not** set an explicit full-keyboard surface with
  `.keyboardViewStyle(.init(background: .color(.keyboardBackground)))`, do not
  repaint `view`/`inputView`/hosting-controller backgrounds, and do not hand-pick
  a gray fallback. Keep those views clear so the keyboard host and KeyboardKit key
  chrome own the surface. The last visible mismatch was the app painting the whole
  keyboard, not just the toolbar.
- Keep Pasta toolbar controls compact and transparent. Prefer icon-only side
  actions around autocomplete, with accessibility labels and no chip/shelf fill.
